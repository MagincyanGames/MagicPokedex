...
export function useCollection() {
  ...
  function getSprite(pokemon: Types.AuthorEntry, form: string | undefined, baseUrl: string | undefined): string | undefined{
    if(!pokemon.base && !form) return undefined 
    return (baseUrl ?? '') + (form && pokemon.forms? pokemon.forms[form] : pokemon.base ?? '')
  }
      ...
      forms: pokemon.forms && Pokemons[key].forms ?
        Object.fromEntries(
          Object.entries(pokemon.forms)
            .filter(([fkey, _]) =>
              Pokemons[key].forms && fkey in Pokemons[key].forms)
            .map(([fkey, _]) =>
              [fkey, getSprite(pokemon, fkey, author?.base_url)]))
        : undefined
    }
  ...
  function GetAuthorPokemons(author: Types.Author | undefined, filter?: { name?: string, region?: string }): Record<string, Types.AuthorEntry> | undefined {
    ...
    return Object.fromEntries(Object.entries(author ? author.pokemons : Pokemons)
      ...
      .map(([key, _]) => [key, GetAuthorSinglePokemon(author, key)!]))
  }
  ...
  function GetShowingSinglePokemon(key: string, form?: string, author?: Types.Author): Types.ShowingPokemon | undefined {
    ...
    return {
      key,
      name: capitalize(pokemon.display ?? key),
      form,
